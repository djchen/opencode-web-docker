...
- **Runtime**: `runtime-config.sh` generates `/runtime-config.js` from container env vars. `OPENCODE_SERVER_URL` is required; `OPENCODE_SERVER_NAME`, `OPENCODE_SERVER_USERNAME`, `OPENCODE_SERVER_PASSWORD` are optional. `OPENCODE_FORCE_DEFAULT_SERVER` defaults to `true`. The script also sets `window.__OPENCODE_SERVER_URL` for the build-time JS patch and removes `location.origin` from the localStorage server store when it differs from `OPENCODE_SERVER_URL`.
